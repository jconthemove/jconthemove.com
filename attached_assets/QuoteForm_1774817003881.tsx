...
interface QuoteFormProps {
  variant?: "customer" | "employee" | "homepage";
...
export default function QuoteForm({ 
  ...
  }, [form, isCustomerPortal, user]);

  const getErrorMessage = (error: Error) => {
    const parts = error.message.split(": ");
    const rawBody = parts.length > 1 ? parts.slice(1).join(": ") : error.message;

    try {
      const parsed = JSON.parse(rawBody);
      if (parsed?.details?.length) {
        return parsed.details.map((detail: { message?: string }) => detail.message).filter(Boolean).join(" ");
      }
      if (parsed?.error) {
        return parsed.error;
      }
  ...
  const submitLead = useMutation({
    ...
    onSuccess: (result: any) => {
      toast({
        ...
        description: isEmployee 
          ...
          : result?.message || (isHomepage
              ? "Your request is in the dispatch queue. We'll contact you shortly."
    ...
    onError: (error: Error) => {
      ...
      toast({
        title: "Error",
        description: isEmployee 
          ? getErrorMessage(error) || "Failed to submit job request. Please try again."
          : isHomepage
            ? getErrorMessage(error) || "Failed to create your job request. Please try again."
            : getErrorMessage(error) || "Failed to submit quote request. Please try again.",
        variant: "destructive",
      });
    },
  });
